...
- \href{https://bookdown.org/content/3686/ordinal-predicted-variable.html}{Cap. 23 - Ordinal Predicted Variable. In. Kurz, S. Doing Bayesian Data Analysis in brms and the tidyverse.}
...
Uma outra forma de pensar sobre esse gráfico é olhar para a relação entre a distribuição das estimativas dos fatores fixos\footnote{Modelos bayesianos apresentam uma série de estimativas para os valores de sigma, de modo que aqui, apenas a título de ilustração, usamos o intervalo de confiança calculado a partir do Erro padrão como a variância da curva.} e os interceptos (tau-cuts $\tau$) estimados pelo modelo: 
```{r, fig.width=5, fig.height=3}
tibble(
  fatores = c("Todo_um", "Singular", "Interação"),
  mu = fixef(m2)[5:7, 1], # Estimativas do modelo
  ci = 1.96*fixef(m2)[5:7, 2]) %>% # Erro padrão x 1.96 (ci = .95)
  expand(nesting(fatores, mu, ci),
         y = seq(from = -5, to = 5, by = .01)) %>%
         mutate(d = dnorm(y, mu, ci)) %>%
  ggplot(aes(x = y, y = d, fill = fatores)) +
  geom_area(alpha = 0.4, position = "identity") +
  geom_vline(xintercept = fixef(m2)[1:4, 1], linetype = "dotted") +
  scale_x_continuous(sec.axis = dup_axis(breaks = fixef(m2)[1:4, 1] %>%
                                           as.double(),
...
E também o gráfico com a distribuição...
```{r, fig.width=5, fig.height=3}
tibble(
  fatores = c("Longa", "Singular", "Interação"),
  mu = fixef(ordinal_mario)[5:7, 1], # Estimativas do modelo
  ci = 1.96*fixef(ordinal_mario)[5:7, 2]) %>% # Erro padrão x 1.96 (ci = .95)
  expand(nesting(fatores, mu, ci),
         y = seq(from = -20, to = 20, by = .01)) %>%
         mutate(d = dnorm(y, mu, ci)) %>%
  ggplot(aes(x = y, y = d, fill = fatores)) +
  geom_area(alpha = 0.4, position = "identity") +
  geom_vline(xintercept = fixef(ordinal_mario)[1:4, 1], linetype = "dotted") +
  scale_x_continuous(sec.axis = dup_axis(breaks = fixef(ordinal_mario)[1:4, 1] %>%
                                           as.double(),
                                         labels = parse(text = stringr::str_c("tau[", 1:4, "]")))) +
  labs(y = "", x = "", fill = "") +
  theme_bw() +
...
\textbf{NOTA DO AUTOR:} Recomendo não discutirmos esses pontos agora. Acho que poderíamos nos aprofundar mais antes de avançarmos aqui. Comecei meus estudos pelas aulas disponíveis no Youtube do Prof. Shravan Vasishth. \href{https://www.youtube.com/watch?v=dhbdG3gXP14&list=PLz_LbfBra_0cvj0zYDGDCLK7CnZ1GGF3j&ab_channel=ShravanVasishth}{A primeira aula} me parece fundamental para entender o que é uma função de verossimilhança (\emph{Likelihood function}). Relembrem também as aulas do Gilberto, em que ele trabalhou esse ponto com bastante atenção, dando vários exemplos. Sem esse conhecimento fica muito difícil avançar para \href{https://www.youtube.com/watch?v=V96A5T6dN6Y&list=PLz_LbfBra_0cvj0zYDGDCLK7CnZ1GGF3j&index=2&ab_channel=ShravanVasishth}{a segunda aula}, em que ele discute a noção de priors, fundamentando os princípios matemáticos por trás dessa noção. Relembrem também a última aula do Gilberto, em que ele trabalhou a noção de priors, com um exemplo de uma moeda não viciada.
